...
g_handles = {
	// system/query.json
	system_query_handle:0,
	// system/about.json
	system_about_handle:0,
	// system/error.json
	system_error_handle:0,
	// system/modules.json
	system_modules_handle:0,
	// job/query.json
	job_query_handle:0,
	// job/query.json
	job_detaile_query_handle:0,
	// system/modules.json
	system_modules_op_handle:0,
	// job/bms.json
	system_bms_handle:0
};
...
function js_init(re_new) {
	...
	g_handles.system_query_handle = setTimeout(js_main_loop, 800);
}

function on_system_query(data, status, xhr){
	if ( status == "success" ) {
		...
		if ( data.version ) {
			...
			if ( data.halt == '急停' ) {
				window.location.href = '/halt.html'
			}
...
// 
function on_system_query_faile (){
...
function js_main_loop() {
	...
	var dt = ' ';
	clearTimeout(g_handles.system_query_handle);
	if ( date.getHours() < 10 ) {
		dt = dt + '0';
	}
	dt = dt + date.getHours() + ':';
	...
	dt = dt + date.getMinutes();
	$('#system_date_time').html(currentdate +  dt);


	$.getJSON(g_cfg.ontom_query, 
			'p=' + g_sys.page_id_curr + '&j=' + g_sys.job_id_curr, 
			on_system_query).fail(on_system_query_faile);
	g_handles.system_query_handle = setTimeout(js_main_loop, 400);
}
...
function page_show_jobs_preview(from) {
	...
	function refresh_jobs_list() {
		...
		clearTimeout(g_handles.job_query_handle);
		g_handles.job_query_handle = setTimeout(refresh_jobs_list, g_cfg.query_period);
	}
	refresh_jobs_list();
}
...
function refresh_module_detail(once) {
	clearTimeout(g_handles.system_modules_handle);
	...
	if ( once != true ) {	
		system_modules_handle = setTimeout(refresh_module_detail, 3000);
	}
}
...
function refresh_current_error_list(once) {
	clearTimeout(g_handles.system_error_handle);
	...
	if ( once != true ) {
		g_handles.system_error_handle = setTimeout(refresh_current_error_list, 1500);
	}
}
...
function do_about_refresh_page() {
	clearTimeout(g_handles.system_about_handle);
	...
	if ( g_sys.page_id_curr =='id_self_info_page' ) {
		g_handles.system_about_handle = setTimeout(do_about_refresh_page, 3000);
	}
}
...
function page_show_self_info(from) {
	...
	g_handles.system_about_handle = setTimeout(do_about_refresh_page, 10);
}
...
function page_show_job_detail(from, job) {
	...
	clearTimeout(g_handles.job_detaile_query_handle);
	...
	function refresh_job_detail() {
		...
		g_handles.job_detaile_query_handle = setTimeout(refresh_job_detail, g_cfg.query_period);
	}
	refresh_job_detail();
}
...
function do_module_op_refresh(p) {
	clearTimeout(g_handles.system_modules_op_handle);
...
/*					codes = codes + "<td>" + d[i].I + "</td>";
	...
	*/				if ( d[i].S != '正常' ) {
	...
	g_handles.system_modules_op_handle = setTimeout(do_module_op_refresh, 2000);
}
...
// 作业页面功能选择响应
function job_options_change(val, from) {
	if ( val == 2 ) {
		page_show_bms_detail(from);
	}
}

// BMS 明细
function page_show_bms_detail(from) {
	$('#'+from).hide();
	$('#id_job_bms_detail_page').show();
	g_sys.page_id_curr = 'id_job_bms_detail_page';
	
	clearTimeout(g_handles.system_bms_handle);

	function refresh_bms_detail() {
		if ( g_sys.page_id_curr != 'id_job_bms_detail_page' ) return;
		$.getJSON(g_cfg.ontom_query_job, '', function (data, status, xhr){
			if ( status == 'success' ) {
				var ok = 0;
				$.each(data, function (index, d) {
					...
					for ( var i = 0; i < d.length; i ++ ) {
						...
						if ( d[i].BRM ) {
							ok ++;
							htl += "<table><caption>BMS.BRM</caption>"
							for ( var n = 0; n < d[i].BRM.length; n += 2 ) {
								htl += "<tr><td>" + d[i].BRM[n].k + "</td><td>" + d[i].BRM[n].v + "</td>";
								
								if ( n + 1 < d[i].BRM.length ) {
									htl += "<td>" + d[i].BRM[n + 1].k + "</td><td>" + d[i].BRM[n + 1].v + "</td>";
								} else {
									htl += "<td>&nbsp;</td><td>&nbsp;</td>";
								}
								htl += "</tr>";
						...
						if ( d[i].BCP ) {
							ok ++;
							htl += "<table><caption>BMS.BCP</caption>"
							for ( var n = 0; n < d[i].BCP.length; n += 2 ) {
								htl += "<tr><td>" + d[i].BCP[n].k + "</td><td>" + d[i].BCP[n].v + "</td>";
								
								if ( n + 1 < d[i].BCP.length ) {
									htl += "<td>" + d[i].BCP[n + 1].k + "</td><td>" + d[i].BCP[n + 1].v + "</td>";
								} else {
									htl += "<td>&nbsp;</td><td>&nbsp;</td>";
								}
								htl += "</tr>";
						...
						if ( d[i].BCS ) {
							ok ++;
							htl += "<table><caption>BMS.BCS</caption>"
							for ( var n = 0; n < d[i].BCS.length; n += 2 ) {
								htl += "<tr><td>" + d[i].BCS[n].k + "</td><td>" + d[i].BCS[n].v + "</td>";
								
								if ( n + 1 < d[i].BCS.length ) {
									htl += "<td>" + d[i].BCS[n + 1].k + "</td><td>" + d[i].BCS[n + 1].v + "</td>";
								} else {
									htl += "<td>&nbsp;</td><td>&nbsp;</td>";
								}
								htl += "</tr>";
						...
						if ( d[i].BCL ) {
							ok ++;
							htl += "<table><caption>BMS.BCL</caption>"
							for ( var n = 0; n < d[i].BCL.length; n += 2 ) {
								htl += "<tr><td>" + d[i].BCL[n].k + "</td><td>" + d[i].BCL[n].v + "</td>";
								
								if ( n + 1 < d[i].BCL.length ) {
									htl += "<td>" + d[i].BCL[n + 1].k + "</td><td>" + d[i].BCL[n + 1].v + "</td>";
								} else {
									htl += "<td>&nbsp;</td><td>&nbsp;</td>";
								}
								htl += "</tr>";
				...
				if ( ok == 0 ) {
					$('#id_bms_detail_panel').html('<a color="#F00">暂无BMS信息</a>');
				}
			}
		});
		g_handles.system_bms_handle = setTimeout(refresh_bms_detail, g_cfg.query_period * 3);
	}
	refresh_bms_detail();
}

function select_bbus_assit () {
	$('#id_assit_select_bbus').css('background-image', 'url(imgs/select.png)');
	$('#id_assit_select_bbus').css('background-color', 'rgba(255,0,0,0.6)');
	$('#id_assit_select_bbus').css('border-style', 'solid');

	$('#id_assit_select_car').css('background-image', 'url(imgs/unselect.png)');
	$('#id_assit_select_car').css('background-color', 'rgba(255,255,255,0.3)');
	$('#id_assit_select_car').css('border-style', 'hidden');
	
	g_commit.cm_assit = 'bbus';
}

function select_car_assit () {
	$('#id_assit_select_car').css('background-image', 'url(imgs/select.png)');
	$('#id_assit_select_car').css('background-color', 'rgba(255,0,0,0.6)');
	$('#id_assit_select_car').css('border-style', 'solid');

	$('#id_assit_select_bbus').css('background-image', 'url(imgs/unselect.png)');
	$('#id_assit_select_bbus').css('background-color', 'rgba(255,255,255,0.3)');
	$('#id_assit_select_bbus').css('border-style', 'hidden');
	
		g_commit.cm_assit = 'car';
}
