...
struct bp_user down_user[] = {
    {500, 0, uart4_simple_box_evt_handle}, // 充电机
    {500, 0, uart4_charger_evt_handle},    // 采样盒
    {0,  0, NULL}
};
// 串口5 使用者为上位机
struct bp_user up_user[] = {
    {500, 0, uart5_background_evt_handle},    // 采样盒
    NULL
};
    ...
    while ( ! *done ) {
        ...
        if ( thiz->status == BP_UART_STAT_WR ) {
            ...
            if ( thiz->tx_param.payload_size ) {
                log_printf(DBG_LV0, "UART: continue becouse: thiz->tx_param.payload_size > 0"s);
                continue;
            }
