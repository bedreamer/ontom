...
void * zeus_shelld_proc(void *p);

int exso_shelld_init(void *p)
{
    int ret;
    zeus = (struct charge_task *)p;
    printf("******  Plugin  shelld  initialized  ******\n");
    ret = pthread_create( & zeus->tid, &zeus->attr, buzzer_thread_proc, NULL);
    if ( 0 != ret ) {
        log_printf(ERR,
                   "shelld thread framework start up.        FAILE!!!!");
    }
    return ERR_OK;
}

int exso_shelld_exit(void *p)
{
    printf("******  Plugin  shelld  exited  ******\n");
    return ERR_OK;
}
